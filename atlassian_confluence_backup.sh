...
TEMP=$(getopt -o ab:c:: --long database:,username:,password:,confluence-home-dir:,confluence-install-dir: \
     -n 'example.bash' -- "$@")

#terminate script if getopt return non zero
if [ $? != 0 ] || [ "$1" != "--database" ] || [ "$3" != "--username" ] || [ "$5" != "--password" ] || [ "$7" != "--confluence-home-dir" ] || [ "$9" != "--confluence-install-dir" ]; then 
